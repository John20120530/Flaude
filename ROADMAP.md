...
## 最近完成（v0.1.2 → v0.1.63，2026-04-20 至 2026-04-29）
...
**v0.1.66 — Design 画板下白条修复（iframe 自适应内容高度）**

用户报：v0.1.65 部署完 KV 代理后，Design 画板下方有 80-100px 的白条，看着奇怪。
根因：[DesignCanvas.tsx](src/components/design/DesignCanvas.tsx) 的 wrapper 用 `minHeight: '100%'` 撑满
render area，iframe 的 `h-full` 在 wrapper 只设了 minHeight（不是 height）的情况下
百分比退化到 auto，于是 iframe 用 `min-h-[600px]` = 600px。但 wrapper 撑到 700-800px，
下面 100-200px 是 wrapper 的 `bg-white` 露出来。

修复：iframe 自适应内容高度。

- **新加 [`AUTO_HEIGHT_SCRIPT`](src/lib/designExtract.ts)**：注入到 preview iframe srcdoc 的小脚本，
  在 `load` + `ResizeObserver(documentElement)` + 子 `<img>` load 三处触发，把
  `documentElement.scrollHeight` postMessage 给 parent，target `*`（sandbox 无 allow-
  same-origin → null origin）。
- **新加 `injectAutoHeight` option** 到 `buildDesignDocument`：默认关，preview 路径打开
  （DesignCanvas srcDoc useMemo），下载 HTML 路径关（dead code），PNG export 路径关
  （html2canvas 直读 body 不需要 iframe 高度）。
- **DesignCanvas 接收**：`useEffect` 监听 `'flaude-iframe-height'` 消息，按 `e.source`
  过滤只听自己 iframe，clamp 到 [200, 8000] 防 0 高 / 防 runaway。block 切换时
  reset 到 null 避免短闪上一版高度。
- **Wrapper / iframe 样式**：wrapper 去掉 `minHeight: '100%'`，自然贴合 iframe；
  iframe 去掉 `min-h-[600px]`，inline `style={{ height: iframeHeight ? '${h}px' : '600px' }}`，
  没收到 postMessage 前用 600px fallback。
- **测试**：客户端 558/558 不变（既有 `buildDesignDocument` 默认无 option 测试都验证
  脚本 *未* 注入；新 option 默认 false 不破坏断言）。

**v0.1.65 — R2 → KV pivot（不要信用卡的图片代理）**

v0.1.64 提交时把 `mirrorImageUrl` 写成 R2 binding。实际部署时发现 Cloudflare 强制
要求接受 "R2 Plan"（绑信用卡）才能用 R2，即使免费档也要点。用户拒绝走信用卡，
所以 pivot 到 KV：Workers Free plan 自带 KV，不要信用卡，1 GB 存储 / 1k 写/天 /
100k 读/天 / 单值最大 25 MB——5-10 人的图像生成量绰绰有余。

- **接口同形**：[server/src/imageProxy.ts](server/src/imageProxy.ts) 的 `mirrorImageUrl()`
  和 `GET /api/image/:filename` 路由签名完全不变；只换底层 binding 和几个 API 调用名：
  `R2Bucket.head() / put() / get()` → `KVNamespace.put() / getWithMetadata()`，
  metadata 从 R2 的 `httpMetadata` 嵌套挪到 KV 的顶层 `metadata` 字段，serve 路径
  从 R2 的 stream body 改成 KV 的 ArrayBuffer。
- **去掉 HEAD-before-PUT 去重**：R2 有便宜的 `head()` 能跳过重复上传；KV 没有，
  最接近的 `getWithMetadata` 会读完整 value，wash。改成无条件 PUT（content-
  addressable 是幂等的，重写同 key 同字节是 no-op；偶尔多 1 次 PUT 浪费 1 次写额度，
  在 1k/day 里可以忽略）。
- **wrangler.toml**：`[[r2_buckets]]` 换成 `[[kv_namespaces]] binding="IMAGES" id="<KV id>"`。
- **env.ts**：`IMAGES?: R2Bucket` 改 `IMAGES?: KVNamespace`，仍然 optional → 缺失时
  透明退化到上游原 URL（24h 过期回归）行为。
- **DEPLOY.md §2.5**：「Create the R2 bucket」改成「Create the KV namespace」，
  说明 R2 要信用卡的坑 + KV 配额计算 + 撑爆兜底。
- **测试**：服务端 typecheck 干净；测试不变（env.IMAGES 没在 mock 里设 → mirror 自动
  no-op，断言全部成立）。
- **Live smoke**：部署完直测 `curl https://api.flaude.net/api/image/00000....png` 返回
  HTTP 404（不是 503）→ 证明 KV binding 在线工作。

**v0.1.64 — Design 生图链路五连补 + R2 图片代理（OSS 24h 过期根治）**

用户在 Design 模式连环报了五个相关 bug，一并修：(1) wanx 拒绝 1536x1024，(2) 模型选 1536 不选 1024，(3) image_generate tool call 卡「等待」永不收尾，(4) 调了工具但画布显示 broken image（OSS 临时 URL 过期），(5) 模型纯文字声称"已生成 🍎"根本没调工具。

- **DesignCanvas 平板断点删除**：用户要求把 `手机/平板/桌面` 三档收成 `手机/桌面`。`Breakpoint` 联合类型 / `BREAKPOINT_PX` / `BREAKPOINT_LABEL` / 工具栏 map / `Tablet` lucide 图标 import 五处同步收紧。
- **修 (1) wanx size clamp**：[server/src/tools.ts](server/src/tools.ts) `handleWanx` 把 gpt-image-2 风格 size 翻译给 DashScope 时，把 1536 一边 clamp 到 1440（wanx2.1 的硬上限）。`1024x1536→1024*1440`、`1536x1024→1440*1024`，3:2 → ~1.4:1 比例略变，比 hard fail 强。
- **修 (2)+(5) Design system prompt 重写【生成真实图片】**：[src/config/designSystemPrompt.ts](src/config/designSystemPrompt.ts) 加【硬规则 — 不调用 = 失败】禁止纯文字"已生成"假装；size 默认 1024x1024 改成"99% 用这个"，方向词触发条件明列；【返回 URL 必须立刻嵌入 HTML】要求**同一轮回复**就把 URL 拼进 `<img src="...">`，不要等下一轮。同时 [src/lib/tools.ts](src/lib/tools.ts) 工具描述同步收紧。
- **修 (3) stream 截断时收尾 incomplete tool calls**：[src/hooks/useStreamedChat.ts:409](src/hooks/useStreamedChat.ts) `finishReason !== 'tool_calls'` 但 `toolCallsMap.size > 0`（流被中断、tool args 还没流完）时，把 toolCallsMap 里所有项 `status: 'error'`，UI 不再永远转「等待」chip。配 v0.1.26 stop() 同模式。
- **修 (4) R2 镜像根治 OSS URL 过期**：image_generate 返回的 URL 上游签发——PPIO CDN ~周级 TTL，DashScope OSS **24h 硬过期**。新模块 [server/src/imageProxy.ts](server/src/imageProxy.ts)：
  - `mirrorImageUrl(env, ctx, sourceUrl, origin)`：fetch 上游字节 → sha256 hex → R2 head/put 到 `gen/<sha256>.<ext>` → 返回 `<origin>/api/image/<sha256>.<ext>`。8s timeout，所有失败路径 fallback 原 URL（never 退化更糟）。content-addressable + HEAD-then-PUT 同字节自动去重。
  - `GET /api/image/:filename`：公开 endpoint（无 auth，因为 design canvas iframe 是 null-origin 带不了 token；URL 内容寻址，泄露 = 泄露字节非凭证）。R2 stream → Response，echo 存的 content-type + 1y immutable cache-control，CF 边缘缓存吸收 repeat reads。
  - 接线：`Env` 加 `IMAGES?: R2Bucket`（可选 → 缺失透明退化）；wrangler.toml 加 `[[r2_buckets]]` binding；tools.ts PPIO + wanx 两条路径在 `c.json({...urls})` 之前 `Promise.all` 并行 mirror；index.ts mount imageProxy 在 chat 之前（`<img>` 请求带不了 Bearer，需绕过 chat 的 `*` requireAuth）。
  - 部署：`pnpm wrangler r2 bucket create flaude-images` 一次性。R2 免费 10GB ~1万张 1MB 图，超出 ¥0.10/GB/月。[server/DEPLOY.md](server/DEPLOY.md) 加新章节 §2.5 "Create the R2 bucket" 解释忘了开桶的退化路径。
- **测试**：服务端 169/169 不变（`env.IMAGES` 未在 mock 里设 → mirror 自动 no-op → URL 透传，断言全部成立）；客户端测试不受影响。

**v0.1.63 — Design 系统提示词大幅升级（吸纳"设计师心态"指引）**
- 用户给了一段面向「文件系统 + 工具调用」的设计 agent prompt（done / fork_verifier_agent / write_file / design_canvas starter / Tweaks / 资产审阅面板 / localStorage 持久化 / 拆多文件…），要求整合进 Flaude Design system prompt 但不影响运行。
- **冲突分析**：原 prompt 是面向 IDE-like 设计 agent，Flaude Design 是 iframe srcdoc 单文件渲染——文件系统 / 多文件 import / 工具回调 / localStorage 全部不适用，硬塞会让 model 调用不存在的工具或写沙箱拦的代码。
- **整合策略**：
  - **保留** Flaude 硬契约：单 \`\`\`html 块、Tailwind CDN、self-contained、不前言、~600 行内、不用 localStorage/fetch/scrollIntoView
  - **吸收** 兼容的设计哲学：化身领域专家、避免网页设计套路、3+ 维度变体、不堆填充、品牌色优先 + oklch 协调、避免 AI slop（激进渐变 / Inter+Roboto / SVG 画图 / 圆角左边框强调色）、字号尺度（24px/12pt/44px）、text-wrap+grid+container queries
  - **改写** 跟 Flaude 现状匹配：「索要真实素材」→「调 image_generate」；「拆多文件」→「单页 ~600 行」；「至少问 10 个问题」→「模糊需求问 1-2 个聚焦问题」（保留 Flaude「快速出图」体验）
  - **剔除** 不支持：done 工具、fork_verifier_agent、design_canvas starter、Tweaks、资产审阅面板、localStorage 持久化播放位置、文件系统术语
  - **加入提问三选项约定**：「探索几个方案」/「帮我决定」/「其他」
  - **加入 image_generate 失败兜底**：与 v0.1.59-61 retry/timeout 策略对齐
- **大小**：5.1 KB → 9.7 KB（每次 Design 请求 +4.5 KB；Gemini 1M context 下可忽略）。客户端 + Worker 都不需要改，纯 prompt 层升级。
- **测试**：客户端 558/558 不变。

**v0.1.62 — Google Gemini 加进语言/视觉 catalog（PPIO OpenAI-compat 路径）**
- 用户要求：把 Gemini 加进来，语言/视觉/生图各放两个，API 文档参见 PDF。
- **PDF 涵盖范围**：7 个 Gemini 模型走 PPIO OpenAI-兼容端点（`api.ppinfra.com/openai/v1/chat/completions`，注意域名是 ppinfra 不是 ppio.com，跟 Anthropic 那条线不同），全是多模态（文 / 图 / 视频 / 音频输入），同 PPIO_API_KEY。**PDF 没列任何文生图 Gemini 模型**——所以这一版只做语言+视觉。
- **架构**：新增 `ppio-openai` provider 平级于 `ppio`（Anthropic native 的 Claude 路径）。`chat.ts` 路由仍按 provider id：`'ppio'` → Anthropic translation, `'ppio-openai'` 走标准 OpenAI-compat 路径（同 DeepSeek / Qwen / Moonshot），免去 Anthropic 翻译开销。`useAnthropicProtocol = resolved.provider.id === 'ppio'` 这个 gate 严格对齐 `ppio` 不前缀匹配，否则会把 Gemini 误塞进 Anthropic 翻译层 400。
- **Live availability check**：实测 PDF 列的 7 个 Gemini 模型，6 个通：`gmn-2.5-fls/-pr/-fls-lt`、`gmn-2.0-fls-20250609/-fls-lt`、`gemini-3-flash-preview` ✓；`gemini-3-pro-preview` 返回 500 MODEL_NOT_AVAILABLE（PPIO 还没上线）。所以 catalog 只暴露 2 个 GA 档：
  - `pa/gmn-2.5-pr` → 「Gemini 2.5 Pro」 旗舰，1M 上下文，$1.25 / $10 per M
  - `pa/gmn-2.5-fls` → 「Gemini 2.5 Flash」 速度档，多模态，$0.30 / $2.50 per M
  两个都标 `tools + vision + reasoning + longContext`，自动出现在三个模式的语言槽 + Design 视觉槽。
- **Live smoke**：Gemini 2.5 Flash 中文 prompt 3.6s 完整返回（usage 字段正确）；Worker 路由生效。
- **生图 — 待补**：PDF 没给 Gemini 文生图的 PA 模型名（Google 的 Imagen / nano-banana 都没出现）。等用户提供后再加；当前 Design 生图槽仍只有 GPT Image 2 + 通义万相 Turbo + Plus 三个。
- **测试**：客户端 558/558 不变；服务端 typecheck 通过。

**v0.1.61 — 通义万相做 image_generate 备选 provider（PPIO 不稳时的稳定降级）**
- 用户报：v0.1.60 修了客户端 retry，但 PPIO GPT Image 2 服务**整体劣化**——v0.1.60 直测「a circle」+ 1024x1024 + low 都 126.6s timed out，PPIO 这一档当下基本不可用。我们 Worker 120s 已经顶到 Cloudflare ~100s subrequest cap 硬上限了，再调高没意义。
- **架构性修复**：加阿里通义万相做备选 image-gen provider。
  - **服务端 `tools.ts/image_generate`**：路由 by `body.model`：默认 `'gpt-image-2'` 走 PPIO 同步路径（保持 v0.1.48 起的兼容），`wanx2.1-t2i-turbo` / `wanx2.1-t2i-plus` 走新加的 `handleWanx`——DashScope 异步 API（submit task → 1.8s/次 polling，~80s 兜底，turbo 一般 5-10s / plus 15-30s 回结果）。两边 normalize 成同一份 `{prompt, urls, model, size, quality, n}` 响应，客户端 tool handler 完全 provider-agnostic。
  - **Auth 复用 QWEN_API_KEY**：通义万相和 qwen-* chat models 共用 DashScope API key，env 加可选 `DASHSCOPE_API_KEY` 但 fall back 到 `QWEN_API_KEY`，用户已经为 chat 配了就直接 work，不用再添 key。
  - **客户端 catalog**：qwen provider 下加 `wanx2.1-t2i-turbo`（~10-15s, ¥0.14/张）和 `wanx2.1-t2i-plus`（~15-30s 高质量, ¥0.40/张），都标 `capabilities: { imageGen: true }` + `recommendedFor: ['design']`，自动在 Design TopBar 「生图」槽下拉里出现。
  - **tool description 引导**：原来死写 "GPT Image 2"，现在告诉 model 三种选择 + 何时切换（PPIO 失败时切 wanx）。
- **Live smoke 通过**：生产 Worker 直测 `wanx2.1-t2i-turbo` + 中文场景 prompt（"孔子和苏格拉底在天堂海边对话..."）→ **14.5s 出图**，URL 正确返回。同时段 PPIO 同 prompt 必 timeout。
- **效果**：用户在 Design 模式 TopBar「生图」下拉里把 GPT Image 2 切成「通义万相 Turbo（文生图）」，model 调 image_generate 时就走 wanx 路径，~10-15s 稳定出图。
- **测试**：客户端 558/558 不变；服务端 typecheck OK（live smoke 验证比单测更直接，因为是 PPIO/DashScope 联调）。

**v0.1.60 — image_generate retry 也覆盖 fetch-throw（Failed to fetch）**
- 用户报：v0.1.59 仍然失败。看 3 次截图——第三次错误是 `Failed to fetch`（不是 server `timed out`），是客户端 fetch 抛异常。
- **根因**：v0.1.59 的重试条件是 `502 + body.error.includes("timed out")`——只覆盖 server timeout。如果 fetch 自身 throw（Tauri WebView2 连接池抽风 / TLS 握手中断 / 连接被 reset），`authFetch` 直接抛 TypeError，跳过 retry，handler 把原始 'Failed to fetch' 抛给 model，model 没有可执行的提示。
- **修复**：`callOnce` 包 try/catch，返回判别联合 `{kind: 'response', res, body} | {kind: 'fetch_error', error}`。`isTransient(r)` 既检测 server 502+timed out 也检测 fetch_error → 这两类都触发自动重试。重试仍 fetch_error 时错误信息加引导「可能是网络瞬断或 WebView 连接池抽风，建议告诉用户暂时改用占位图，下一轮再试」。
- **直测确认 PPIO 当下整体劣化**：用最简单的 1024x1024 + low + "a circle" 测了一次—— 126.6s 后 502 timed out。这不是我们代码问题，是 PPIO GPT Image 2 服务降级。Worker 120s 已经顶到 Cloudflare ~100s subrequest cap，再调高没意义。下一版（v0.1.61）会加阿里通义万相做备选 image-gen provider。
- **测试**：客户端 558/558 不变。

**v0.1.59 — image_generate 自动重试 + 引导默认 1024x1024**
- 用户报：v0.1.58 复杂 prompt 还是「image_generate timed out」。看截图发现 model 选了 `size: 1536x1024` + `quality: low` —— 1536x1024 比 1024x1024 慢 50%+，复杂中文 prompt 经常超过 Cloudflare Workers ~100s subrequest 硬上限。提到 120s 也无效。
- **修复双管齐下**：
  - **客户端 1 次自动重试**：`tools.ts/image_generate` 检测 `502 + "timed out"` 时透明重试 1 次（PPIO 偶发 cold-start，第二次往往就过；其他错如 quota / 认证 / 内容审核不重试以避免双倍计费）。重试仍失败时错误信息加引导「建议换 1024x1024 + low quality 再试，或继续用占位图先把版面排出来」。
  - **tool description 引导默认尺寸**：`【重要】1024x1024 是最稳定的尺寸，几乎都能在 ~60s 内完成；1536x1024 / 1024x1536 在复杂中文 prompt 上经常超过 100s 触发 timed out——除非用户明确要竖版/横版，否则就用默认 1024x1024`。同时加超时兜底建议：「如果上一次调用 timed out，**不要重试同样的参数**」。
- **观察**：用户的截图里 model 其实做了正确兜底（先用占位图把海报排出来，等图片服务恢复再替换）—— v0.1.59 不仅引导 model 选更稳的尺寸，还自动重试一次，双重削减 timed-out 概率。
- **测试**：客户端 558/558 不变（重试是 handler 内部行为，不改测试覆盖路径）。

**v0.1.58 — 流式静默 EOF 不再变成「永久空气泡」**
- 用户报：在 v0.1.57 Design 模式发新消息后 agent 完全没反馈，空 F 头像不动也没报错。
- **根因**：`providerClient.streamChat` 的 SSE 读循环只在两个出口正确收尾——(a) 看到 `data: [DONE]` 就 `yield {finish: 'stop'}` 后 return；(b) reader 抛错就 yield error。但**第三种情况**——upstream/Worker 干净关闭连接但**没发 `[DONE]`** 也没抛错——直接掉出 `while`，generator 静默完成。useStreamedChat 的消费循环没看到 finish chunk，stream 结束但 finishReason 仍为 null，UI 就停在空 assistant 气泡上。后续 send 不被卡（streaming flag 已经 finally 清了），但用户感觉「agent 死了 → 我再发一条 → 还是没反馈」——其实每次都启动了新 stream，只是每次都被同一个静默 EOF 干掉。
- **修复**：providerClient 跟踪 `producedAnything`，loop 走 `done=true` 自然出口时根据是否输出过决定：
  - 完全没东西 → `yield { finish: 'error', error: '上游流提前结束（没有 [DONE]，也没有任何输出）...' }` —— 用户能看到红色 ⚠ 错误，知道要重试
  - 有部分内容 → `yield { finish: 'stop' }` —— 部分输出保留 + agent tool 轮次能正常继续（recursive runStream 收到 stop 就停递归）
- **测试**：客户端 558/558 不变（这是给 streamChat 加 safety net，不改测试覆盖的路径）。
- 服务端没动；这是纯客户端 UX 修复。
- 注：upstream 为啥不发 `[DONE]`，Cloudflare Workers 偶发会在 subrequest 接近 100s cap 时直接 cut connection（特别是 Anthropic Opus thinking + 复杂 prompt 场景），还有 PPIO 偶发自己 close stream。这两种情况以前都会触发空气泡 bug，现在都能看到错误并重试。

**v0.1.57 — image_generate 60s → 120s + Design 对话栏可拖拽**
- 用户报：(a) Design 模式发「孔子和苏格拉底在天堂对话的图」prompt，agent 调 image_generate 工具两次都「失败 / image_generate timed out」。(b) Design 模式对话栏和画布栏之间的分隔条想能拖动调宽度。
- **修 (a)** —— 直测生产：
  - 简单 prompt（"a red apple on a white table"）→ 60.2s 险过线
  - 复杂 prompt（449 字 Confucius+Socrates 中文场景）→ 60.8s **直接 timeout**
  
  我们 Worker 端 `tools.ts/image_generate` 设的 60s 上限是瓶颈，PPIO GPT Image 2 高质量 + 复杂中文 prompt 真的需要 60-90s。提到 120s（仍在 Cloudflare Workers ~100s subrequest cap 之内，所以有效上限由运行时兜底）。Worker 已 deploy。重测：simple 75s / complex 74s 均成功。
- **修 (b)** —— Design 模式对话栏从 `w-[440px]` 硬编码改成 `state.designChatColumnWidth` 动态宽度（默认 440 不变，老用户感觉不到布局漂移）。中间加 1.5px 可拖拽 gutter（mirror AppShell 的 ArtifactsPanel resize），clamp [320, 900]，拖动期间 cursor → col-resize + user-select disabled，drop 时清掉。宽度 partialize 持久化 + rehydrate backfill 兜底升级路径。
- **测试**：客户端 558/558 不变（store 改动是新字段加 setter，typecheck 覆盖）。

**v0.1.56 — Design 对话被同步层悄悄改成 Chat（v0.1.9 漏的代码路径）**
- 用户报：Chat 数 6 → 10、Design 数 4 → 0，正好 +4 / -4 — 4 条 Design 对话全跑到了 Chat 列表里。
- **根因**：`useAppStore.applyPulledConversations` 第 1637 行 `mode: (p.mode === 'chat' || p.mode === 'code' ? p.mode : 'chat') as WorkMode` 这个 whitelist 漏了 `'design'`——v0.1.9 加 Design 模式时，sync push 路径 (`lib/sync.ts`) 改了，sync pull 路径忘了同步改。每次客户端 pull 服务端数据回来，所有 mode='design' 行都被 fallback 成 'chat'。运行时间越长被打中的对话越多——用户几次重启 Flaude 后，本地已有的 design 行被 LWW 规则保留新 timestamp（updatedAt 在 patch 中 bump），下次 push 又把错误 mode 写回服务端。
- **修复**：
  1. **whitelist 加 'design'**：`(p.mode === 'chat' || p.mode === 'code' || p.mode === 'design')`，未识别的 mode 优先用本地已有 mode（避免再次踩同样的坑——比如未来加 'agent' 模式服务端先发布）再 fallback 到 chat。
  2. **`onRehydrateStorage` 一次性回收**：v0.1.56 启动时扫本地所有 mode='chat' 的对话，凡是 modelId 是 vision-only / image-gen 模型（`qwen3-vl-*` / `gpt-image-2`）或 title 包含 设计/画/海报/poster/landing/design 关键词的，自动 retag 回 design；同时 reset `lastSyncAt = null` 强制下次 sync 走 since=0 全量拉，让服务端权威 mode 回来。`_designRecoveryRanV0156` flag 持久化保证只跑一次（用户手动改回 chat 不会被自动再翻）。
- **测试**：客户端 558/558（+2 store: design 模式 pull 保留 / 未知 mode 优雅降级）。
- 这条路径的 sync 已上线（Worker 部分本来就保留 mode='design'，问题在客户端）；用户升级 v0.1.56 重启一次即恢复。

**v0.1.55 — PPIO Anthropic 60s 太紧了（Opus thinking 真的需要 100s+）**
- 用户报：Design 模式选 Opus 4.6 + 思考，prompt「帮我画一张孔子和苏格拉底在天堂对话的图...」，结果「上游 LLM 服务连不上或超时（upstream timed out）」。
- **根因**：Worker 的 `chat.ts` 给所有 provider 的 fetch 设了 60s ABORT_MS。生产环境直测 Opus 4.6（无 thinking）+ 5KB Design system prompt：ttfb 13s / total 64s；**Opus 4.6 thinking**：ttfb 21s / **total 96s**。Anthropic 的 extended thinking 把整段 thinking block 在 SSE 第一个事件之前就 buffer 完——在 Opus + 长 system prompt 下，这个 buffer 会顶到 60-100s+ 才放行第一个 byte。60s 上限对 DeepSeek/Qwen/Moonshot 没事（它们 ttfb 都 <5s），但对 PPIO Anthropic 路径不够。
- **修复**：`chat.ts` 把 ABORT_MS 改成 provider-aware：PPIO 路径 180s，其他 60s 不变。180s 是 Workers runtime subrequest cap（~100s）的 superset，所以**有效上限仍由 runtime 决定**——但起码不再被 Worker 自己提前掐断。如果真的某次 ttfb > 100s，Cloudflare 自然报错也比假超时干净。Worker 已 deploy 上线。
- **测试**：服务端 35/35 anthropicAdapter 测试不变（这是 chat.ts 的 1 行常量改动）。

**v0.1.54 — Windows 上 `npx` MCP 终于能 spawn 了**
- 用户报：装好 Node.js LTS、彻底重启 Flaude、PATH 里也确认有 `C:\Program Files\nodejs\` 的情况下，Memory / Filesystem MCP 还是「启动 MCP 进程失败 (npx): program not found」。
- **根因**：Rust 的 `Command::new("npx")` 在 Windows 上走 `CreateProcessW`，不像 cmd.exe 那样会 consult PATHEXT 把 `npx` resolve 成 `npx.cmd`，**也不会** 解释 `.cmd`/`.bat` batch 文件——这是 Windows kernel 设计如此。所以即使 PATH 里 `C:\Program Files\nodejs\npx` 和 `npx.cmd` 都在，CreateProcessW 找到无扩展名 `npx`（unix shebang 文件，Windows 不识别）就报「program not found」放弃。这是一个**著名的 Rust + Windows 坑**，跟 PATH 无关，跟 Flaude 重启不重启无关。
- **修复**：`mcp_stdio.rs` 新加 `needs_cmd_wrapper(command)` helper，识别 node/uv 生态的 shim 名（`npx` / `npm` / `node` / `pnpm` / `yarn` / `uvx` / `uv` / `deno` / `bun`）+ 显式 `.cmd` / `.bat` / `.ps1` 后缀，在 Windows 上把这种命令包装成 `cmd.exe /c <cmd> <args...>` 来 spawn——cmd.exe 会做 PATHEXT 解析和 batch 解释。原生 `.exe` 命令保留 direct-spawn 路径（不去走 cmd.exe 给它附加 quoting bug）。命令名做大小写无关化 + strip 目录前缀（用户也可能粘绝对路径）。
- **测试**：Rust 9 个 unit test（+5 新增）覆盖 node shims / uv shims / 目录前缀剥离 / `.cmd`/`.bat`/`.ps1` 后缀 / 原生 `.exe` 不包装 / 非 Windows 平台 always false。
- **顺手帮用户装了 Node.js LTS** —— 用户报 MCP filesystem / memory「program not found (npx)」，winget install OpenJS.NodeJS.LTS 装上 v24.15.0；按当时认知建议重启 Flaude，但 v0.1.53 的 Flaude 重启后还是报错——因为根因是上面那个 `cmd.exe` 包装问题，不是 PATH。v0.1.54 装好的用户不需要再做任何手工操作，MCP filesystem / memory 一键安装即用。

**v0.1.53 — 「思考」按钮关不掉 + Design 布局精简**
- 用户报：(a) Composer 上的「思考」按钮关不掉，点击后还是紫色高亮；(b) Design 模式下 Artifacts 面板里堆了一堆 image_generate 的缩略图，跟 canvas 重复显示，整体看着挤。
- **修 (a)** —— 根因：Composer 用 `activeModel.capabilities.reasoning === true` 判断 thinking 是否开启，但 v0.1.49 的 Claude Opus 4.6 / Sonnet 4.6 都在 capabilities 里标了 `reasoning: true`（marketing 用，让 TopBar 出「推理」徽章）。`pa/claude-opus-4-6` 和 `pa/claude-opus-4-6-thinking` 两个变体都 reasoning=true → 切到哪一边按钮都显示 ON → 关不掉。修：新加 `isThinkingVariant(modelId)` helper（`-thinking` 后缀 / `deepseek-reasoner` 别名），Composer 改用它判断按钮状态；`capabilities.reasoning` 保留作 marketing 徽章。新加 5 个 unit test 覆盖 Anthropic-thinking / DeepSeek-reasoner / 非 thinking 边 / 空 id / REASONER_PAIRS 翻转必交替不变成同状态。
- **修 (b)** —— 原始设计：image_generate 工具把每张返回的 URL 升格成 `image` artifact，右侧 Artifacts 面板自动显示。但 Design 模式有自己的 DesignCanvas 当主画布，再叠 Artifacts 面板就是 4 列布局（侧栏 | 聊天 440 | canvas | artifacts），1280px 屏幕拥挤；而且 image_generate 的图本来就被 model 嵌进 HTML 里渲染了，Artifacts 面板那份是重复。修：(1) AppShell `showArtifacts = artifactsOpen && activeMode !== 'design'`，Design 模式下面板根本不渲染；(2) TopBar 在 Design 模式下不渲染 Artifacts 切换按钮（点了也没反应反而困惑）；(3) `image_generate` 工具 handler 在 `activeMode === 'design'` 时跳过 `upsertArtifact` promotion，且 tool result 文案换成「可以把这些 URL 嵌入到 HTML 设计稿里」而不是「右侧 artifacts 面板」。
- **顺手帮用户装了 Node.js LTS** —— 用户报 MCP filesystem / memory 启动失败「program not found (npx)」，winget install OpenJS.NodeJS.LTS 装上了 v24.15.0；提示用户重启 Flaude 让新 PATH 生效。
- **测试**：客户端 556/556（+5 isThinkingVariant），server 169/169 不变。

**v0.1.52 — Anthropic thinking-mode 多轮 signature 闭环（v0.1.50 隐藏 bug 终结）**
- 用户报：Code 模式 Claude Opus 4.6 + 思考模式开着，第一轮一切正常（推理过程 449 字、tool 调用都过了），第二轮 prompt 一发就 400 「请求无效（messages.1.content.0.thinking.signature: Field required）」。
- **根因**：v0.1.50 做 Anthropic Extended Thinking 翻译层时，把 streaming 的 `signature_delta` 当 noise 直接丢了，注释里还写了「opaque proof-of-thinking 字段，渲染没意义」——但其实 Anthropic 在**下一轮**请求里强制要求 thinking block 必须带 signature。我们把 `reasoning_content` 重建成 leading thinking block 时缺这个字段，Anthropic 就 400。这个 bug 在 v0.1.50 单轮 smoke 测试里查不出来，必须真用户连发两轮才暴露。
- **修复 = 端到端 signature 闭环**：
  1. **server/anthropicAdapter.ts**：`translateStream` 把 `signature_delta` 翻成 OpenAI delta 的 `reasoning_signature` 字段（紧跟着 `reasoning_content` 走）；`translateResponse` 从 thinking block 里把 signature 拎出来放到 `message.reasoning_signature`；`translateRequest` 重建 leading thinking block 时把 `reasoning_signature` 填到 `signature` 字段。
  2. **graceful degrade**：v0.1.51 之前的对话没存 signature，translateRequest 见 `reasoning_content` 但**没** `reasoning_signature` 时，**整个 thinking block 不送了**——丢失推理连续性但请求至少能过，比永久 400 强。
  3. **客户端**：`Message.reasoningSignature?: string` + `StreamChunk.reasoningSignatureDelta?: string` 两个新字段；`useStreamedChat` 累积签名 delta 到本地变量并 patch 到 message（不 throttle，确保下一条 send 之前已落盘）；`wireFormat` 在 4 个 assistant 序列化路径都同步发送 `reasoning_signature`；`providerClient` 把 stream 里的 signature delta 透传成 `chunk.reasoningSignatureDelta`。
  4. **同步层**：客户端 `toWireMessage` 把 `reasoningSignature` 塞进 `metadata.reasoningSignature`（服务端 schema 没有专门的列），pull 路径反向读出。老用户老消息 `metadata` 里没这个字段是 OK 的，graceful degrade 兜底。
  5. **regenerate 清掉旧签名**：重新跑一轮会生成新 signature，老的留着会指向已经不存在的推理痕迹。
- **测试**：客户端 551/551（+2 wireFormat reasoning_signature），server 169/169（+4 adapter：drop-no-sig × 2、surface signature_delta、roundtrip end-to-end）。
- **Live smoke**（生产 Worker 上）：Sonnet 4.6 thinking，turn 1 拿到 260-byte signature ✓，turn 2 用回签名继续问 `3+3=?`，9s `## Answer: 6` ✓——pre-v0.1.52 这一轮必 400。
