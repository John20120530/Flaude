...
const BUILTIN_TOOLS: ToolDefinition[] = [
  ...
  // mirrors how `create_artifact` works for HTML/SVG.
  {
    ...
    description:
      ...
      '【强制】用户要求"画/生成/做"图片时**必须**真的调用这个工具——禁止纯文字回"已生成"而不发起调用，那样用户什么都看不到。' +
      '【尺寸硬规则】**默认 1024x1024**。1024x1024 是 GPT Image 2 + 通义万相唯一都稳定支持的尺寸；1536x1024 / 1024x1536 在 GPT Image 2 上经常超过 100s 触发 timed out，**且通义万相会被自动 clamp 到 1440 长边导致比例略变**。除非用户明说"横版/竖版/banner/海报横构图"等方向词，否则**永远用 1024x1024**——不要自作主张选 1536。' +
    ...
    parameters: {
      ...
      properties: {
        ...
        size: {
          ...
          description:
            '图片尺寸：**99% 情况都应该用 1024x1024**（GPT Image 2 + 通义万相都稳定支持，最快不超时）。仅当用户明说"横版/竖版/banner/海报横构图/封面竖图"等方向性词汇时，才考虑 1536x1024（横）或 1024x1536（竖）——但通义万相会自动 clamp 到 1440 长边。auto 让模型决定，慎用。',
        },
        quality: {
          type: 'string',
